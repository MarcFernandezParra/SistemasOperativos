...
void readcsv() {
  ...
  char* name;
  char types[2][32];
  int id, total, hp, attack, defense, spAttack, spDefense, speed, generation, legendary, seen, captured;
  ...
  while (fgets(buf, 151, f) != NULL) {
    ...
    pokedex[i] = new_pokemon(pokemon_id, name, types, total, hp, attack, defense, 
                             spAttack, spDefense, speed, generation, 
                             legendary, seen, captured);
